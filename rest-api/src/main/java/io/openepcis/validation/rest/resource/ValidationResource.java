...
public class ValidationResource {
  ...
  @RequestBody(
      description = "Validate EPCIS 2.0 JSON/JSON-LD/XML or EPCIS 1.2 XML document.",
      content = {
        @Content(
            mediaType = MediaType.APPLICATION_JSON,
  ...
  public Uni<RestResponse<?>> validateEvents(
      ...
      @QueryParam("epcisDocumentSchema")
          ...
          @Schema(
              ...
              enumeration = {EPCIS.CAPTURE, EPCIS.QUERY})
    ...
    return schemaValidator
        .validate(
            body,
            contentType,
            EPCISDocumentType.fromString(epcisDocumentSchema),
            EPCISVersion.fromString(epcisVersion).get())
            ...
            validationErrors -> {
              ...
              if (MediaType.APPLICATION_XML.equals(accept)) {
                return Uni.createFrom()
                    .item(
                        RestResponse.status(
                            Response.Status.BAD_REQUEST, new ValidationResult(validationErrors)));
              }
              return Uni.createFrom()
                  .item(RestResponse.status(Response.Status.BAD_REQUEST, validationErrors));
            })
        .onFailure()
        .recoverWithItem(this::createErrorResponse);
  }
